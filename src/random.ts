...
export async function findRandomRepo() {
	...
	try {
		...
		if (!ghData.items?.length) {
			throw new Error(
				`No results for "${randomWord.word}" on page #${pageInput.value}. Please try again!`,
			);
		}
		...
		ghData.items.forEach((repo: any) => {
			...
			const numInfo = repo.stargazers_count + repo.forks_count;

			div.innerHTML = `
<a href="${repo.html_url}" target="_blank">${repo.full_name}</a>
${repo.description ? `<p>Description: ${repo.description}</p>` : ""}
${numInfo ? `<p>Stars: ${repo.stargazers_count.toLocaleString()} | Forks: ${repo.forks_count.toLocaleString()}</p>` : ""}
